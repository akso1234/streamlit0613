# --- START OF MODIFIED FILE pages/1_Hospital.py (지도 표시 원복) ---
...
def run_hospital_page():
    ...
    with tab1:
        ...
        merged_avg = make_merged_avg_beds(df_hosp, df_beds, gdf_gu) # 원본 변수명 사용
    ...
    with tab3: 
        st.subheader(f"🌡️ {selected_year}년 평균 병상 수 히트맵") # 원본 탭 제목/순서 유지
    ...
    with tab4: 
        st.subheader(f"🛏️ {selected_year}년 구별 전체 병원, 병상 그래프") # 원본 탭 제목/순서 유지
        if df_hosp is not None and not df_hosp.empty and \
           df_beds is not None and not df_beds.empty:
