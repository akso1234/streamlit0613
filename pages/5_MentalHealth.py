...
# utils.py와 chart_utils.py가 프로젝트 루트에 있다고 가정합니다.
...
from chart_utils import (
    ...
    # 새로 추가될 그래프 함수 (chart_utils.py에 이미 정의되어 있어야 함)
    plot_all_conditions_trend_lineplot,
    plot_elderly_population_ratio_trend_lineplot,
    plot_sigungu_mental_patients_ratio_by_condition_year # 구별 '환자 비율' 그래프
)
...
def calculate_seoul_total_mental_health_ratios(combined_year_df, seoul_total_elderly_pop_final):
    ...
    # '총_65세_이상_노인인구' 컬럼이 병합 후에도 있는지 확인
...
def calculate_sigungu_mental_health_ratios(df_sigungu_mental_total_patients, df_sigungu_total_elderly_population):
    ...
    if '총_65세_이상_노인인구' not in df_sigungu_ratio_final.columns:
        ...
        df_sigungu_ratio_final['분모_전체_노인인구'] = 0 # 오류 방지를 위해 컬럼 추가
    else:
    ...
    if '분모_전체_노인인구' in df_sigungu_ratio_final.columns:
        ...
        df_sigungu_ratio_final['전체노인인구_대비_질환자_비율(%)'] = 0 # 분모가 없으면 비율은 0
...
def run_mental_health_page():
    ...
    # 세션 상태 키 이름 변경 (탭별로 구분)
    if "selected_year_mental_tab_overall" not in st.session_state:
        st.session_state.selected_year_mental_tab_overall = available_years_for_mental_analysis[-1]
    if "selected_year_mental_tab_sigungu_ratio" not in st.session_state:
        st.session_state.selected_year_mental_tab_sigungu_ratio = available_years_for_mental_analysis[-1]

    dataframes_by_condition = {}
    all_conditions_summaries_list = [] # 서울시 전체 질환별, 연도별 환자수 집계용
    ...
    tab_titles = [
        "개별 질환 분석 (서울시 전체)", 
        "서울시 전체 종합 비교", # 기존 "정신질환 종합 비교" 탭
        "구별 환자 수",        # 기존 "구별 정신질환자 수" 탭
        "서울시 전체 비율 분석", # Notebook Cell 14 내용 (신규 탭)
        "구별 환자 비율 분석"  # Notebook Cell 22 내용 (신규 탭)
    ]
    ...
    with main_tab2: # 서울시 전체 종합 비교
        current_selected_year_tab2 = st.session_state.selected_year_mental_overall # 세션 키 이름 변경
        st.subheader(f"{current_selected_year_tab2}년 {tab_titles[1]}") # 탭 제목 사용
        ...
        selected_year_val_tab2 = st.slider(
            ...
            step=1,
            key="mental_year_slider_overall_page5_v2" # 키 변경
        )
        if st.session_state.selected_year_mental_overall != selected_year_val_tab2:
            st.session_state.selected_year_mental_overall = selected_year_val_tab2
            st.rerun()
    ...
    with main_tab3: # 구별 환자 수
        current_selected_year_tab3 = st.session_state.selected_year_mental_sigungu_ratio # 이 탭은 비율이 아닌 환자 '수'
        st.subheader(f"{current_selected_year_tab3}년 {tab_titles[2]}") 

        selected_year_val_tab3 = st.slider(
            "조회 연도 선택 ", # 뒤에 공백 추가하여 이전 슬라이더와 구분
            min_value=min(available_years_for_mental_analysis),
            max_value=max(available_years_for_mental_analysis),
            value=st.session_state.selected_year_mental_sigungu_ratio, # 임시로 같은 세션 상태 사용, 필요시 분리
            step=1,
            key="mental_year_slider_sigungu_count_page5_v2" # 키 변경
        )
        if st.session_state.selected_year_mental_sigungu_ratio != selected_year_val_tab3: # 임시로 같은 세션 상태 사용
            st.session_state.selected_year_mental_sigungu_ratio = selected_year_val_tab3
            st.rerun()
        ...
        selected_condition_tab3 = st.selectbox(
            ...
            key="mental_condition_select_sigungu_count_page5_v2" # 키 변경
        )
        
        if selected_condition_tab3 != "데이터 없음" and dataframes_by_condition and selected_condition_tab3 in dataframes_by_condition:
            if not df_sigungu_mental_total_patients.empty: # df_sigungu_mental_total_patients_for_tab3 대신 사용
                plot_sigungu_mental_patients_by_condition_year( # 이 함수는 chart_utils에 있어야 함
                    df_sigungu_mental_total_patients, 
    ...
    with main_tab4: # 서울시 전체 비율 분석 (신규 탭)
        ...
        if not combined_year_df_for_seoul.empty:
            ...
            plot_all_conditions_trend_lineplot(combined_year_df_for_seoul) # chart_utils.py 에 정의 필요
            st.markdown("---")
        else:
        ...
        if not df_merged_ratio_seoul_total_final.empty:
            ...
            plot_elderly_population_ratio_trend_lineplot(df_merged_ratio_seoul_total_final) # chart_utils.py 에 정의 필요
        else:
            st.info("서울시 전체 노인 인구 대비 환자 비율 추이 데이터를 계산하거나 표시할 수 없습니다.")


    with main_tab5: # 구별 환자 비율 분석 (신규 탭)
        current_selected_year_sigungu_ratio = st.session_state.selected_year_mental_sigungu_ratio
        st.subheader(f"{current_selected_year_sigungu_ratio}년 {tab_titles[4]}")

        selected_year_sigungu_ratio_val = st.slider(
            "조회 연도 선택  ", # 레이블에 공백 추가로 구분
            min_value=min(available_years_for_mental_analysis),
            max_value=max(available_years_for_mental_analysis),
            value=st.session_state.selected_year_mental_sigungu_ratio,
            step=1,
            key="mental_year_slider_sigungu_ratio_page5_v3"
        )
        if st.session_state.selected_year_mental_sigungu_ratio != selected_year_sigungu_ratio_val:
            st.session_state.selected_year_mental_sigungu_ratio = selected_year_sigungu_ratio_val
            st.rerun()
        ...
        selected_condition_sigungu_ratio = st.selectbox(
            "분석할 질환 선택  ", # 레이블에 공백 추가
            selectbox_options_tab5,
            key="mental_condition_select_sigungu_ratio_page5_v3"
        )
