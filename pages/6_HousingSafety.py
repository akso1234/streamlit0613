...
def preprocess_elderly_data_for_housing_cached(df_elderly_raw):
    ...
    try:
        ...
        if gu_info_col_tuple_elderly in df_filtered_elderly.columns and \
...
def preprocess_housing_data_cached(df_housing_raw):
    ...
    try:
        ...
        gu_name_column_actual_tuple = df_housing_processed.columns[1] 

        df_filtered_housing = df_housing_processed[
            df_housing_processed[gu_name_column_actual_tuple].astype(str).str.strip() != '소계'
        ].copy()
...
def plot_bubble_chart_ratio(df_final_merged, target_cause_for_bubble):
    ...
    ax.set_xlabel('30년 이상 노후 주택 비율', fontsize=12); ax.set_ylabel('고령 인구 비율 (65세 이상)', fontsize=12)
    ax.xaxis.set_major_formatter(PercentFormatter(1.0, decimals=0))
...
def plot_heatmap_housing_elderly_incident_ratio(df_merged_ratio, accident_col_name, target_safety_accident_cause):
    ...
    try:
        ...
        if not heatmap_data_ratio.empty:
            ...
            ax_heatmap.set_xlabel('30년 이상 노후 주택 비율 구간', fontsize=12)
            ...
            if selected_cause_corr_ratio:
                ...
                if not merged_df_corr_elderly_ratio.empty:
                    plot_correlation_scatter_ratio(merged_df_corr_elderly_ratio, '고령인구비율', f'{selected_cause_corr_ratio}건수', 
                                                 f"고령 인구 비율과 {selected_cause_corr_ratio} 발생 건수", "고령 인구 비율 (65세 이상)")
                ...
                if not merged_df_corr_housing_ratio.empty:
                    plot_correlation_scatter_ratio(merged_df_corr_housing_ratio, '노후주택비율', f'{selected_cause_corr_ratio}건수', 
                                                 f"노후 주택 비율과 {selected_cause_corr_ratio} 발생 건수", "30년 이상 노후 주택 비율")
                else: st.info("노후 주택 비율 데이터와 사고 건수 병합 결과 데이터가 없습니다.")
        else: 
            missing_info = []
            if '사고원인' not in df_rescue_processed.columns: missing_info.append("'사고원인' 컬럼")
            if '고령인구비율' not in df_elderly_processed_h.columns: missing_info.append("'고령인구비율' 컬럼")
            ...
            if not df_housing_processed_h.empty and '노후주택비율' in df_housing_processed_h.columns and \
               not df_elderly_processed_h.empty and '고령인구비율' in df_elderly_processed_h.columns:
                ...
                if not df_merged_bubble_ratio_step1.empty:
                    ...
                    else: st.info(f"'{cause_for_bubble_ratio}' 사고 기준 종합 분석용 최종 데이터(비율 기반)가 비어있습니다.")
                ...
                st.info(f"다음 데이터가 준비되지 않아 비율 기반 버블 차트/히트맵 분석을 수행할 수 없습니다: {', '.join(missing_info_bubble)}")
