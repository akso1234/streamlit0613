...
def preprocess_elderly_data_for_housing_cached(df_elderly_raw):
    ...
    try:
        ...
        if isinstance(df_elderly_processed.columns, pd.MultiIndex) and len(df_elderly_processed.columns.levels) >= 4: 
            for col_tuple in df_elderly_processed.columns:
                if len(col_tuple) == 4: 
                    ...
                    if c0.startswith('2023'): # 특정 연도 가정 (필요시 동적으로 변경)
            ...
            # MultiIndex가 아니거나 레벨이 다른 경우의 처리 (단순 컬럼명으로 시도)
            found_target = False
            found_total = False
            for col_name_str in df_elderly_processed.columns:
                if isinstance(col_name_str, str): # 단순 문자열 컬럼명일 경우
                    if '2023' in col_name_str and '65세이상 인구' in col_name_str and '소계' in col_name_str: # 좀 더 유연한 검색
                        target_pop_col_tuple = col_name_str # 문자열도 튜플처럼 취급
                        found_target = True
        ...
        if isinstance(df_elderly_processed.columns, pd.MultiIndex):
            gu_info_col_name = df_elderly_processed.columns[1] # 두번째 컬럼이 자치구 정보라고 가정
        else: # 단일 레벨 컬럼
            if len(df_elderly_processed.columns) > 1:
                 gu_info_col_name = df_elderly_processed.columns[1] # CSV와 유사하게 두번째 컬럼 가정
            else:
...
def preprocess_housing_data_cached(df_housing_raw):
    ...
    try:
        ...
        year_prefix_housing = '2023' # 특정 연도 가정 (필요시 동적으로 변경)      
        ...
        if isinstance(df_housing_processed.columns, pd.MultiIndex) and len(df_housing_processed.columns.levels) >= 3:
            for col_tuple_h in df_housing_processed.columns:
                if len(col_tuple_h) == 3:
                    ...
                    if c0_h.startswith(year_prefix_housing):
                        if c1_h in ["30년이상", "30년 이상"] and c2_h == "계": # "30년 이상" 도 추가
                            target_old_housing_col_tuple = col_tuple_h
                        # '20년이상 30년미만' 컬럼을 찾기 위한 조건 추가
            ...
            if col_20_to_30_total_tuple is None :
                 ...
                 return pd.DataFrame()
        else: # MultiIndex가 아닌 경우의 처리 (단순 컬럼명으로 시도)
        ...
        # '전체주택수'를 CSV의 '전체 주택' 또는 다른 모든 기간의 합으로 계산해야 함
        # 현재는 20-30년 + 30년 이상만 더하고 있음.
        # 더 정확한 '전체주택수'를 위해 모든 연령대 주택 수 컬럼을 찾아 합산하는 로직이 필요할 수 있음.
        # 예시: ('2023', '주택총수', '계', '계') 또는 ('2023', '합계', '주택유형별', '계', '소계') 같은 컬럼
        # 지금은 제공된 컬럼만으로 계산합니다.
        df_final_housing['전체주택수'] = df_final_housing['주택수_20년_30년미만'] + df_final_housing['노후주택수'] # 이 계산이 정확한지 데이터 확인 필요
...
def plot_gu_incident_counts(df_rescue):
    ...
    ax.set_ylabel('사고 건수 (개)', fontsize=12) # 요청사항 1
...
def plot_stacked_bar_incident_causes_by_gu(df_rescue, top_n_causes=7):
    ...
    ax.set_xlabel('자치구', fontsize=12)
    ax.set_ylabel('사고 건수 (개)', fontsize=12) # 요청사항 2
    plt.setp(ax.get_xticklabels(), rotation=45, ha="right", fontsize=10); ax.tick_params(axis='y', labelsize=10)
...
def plot_pie_major_incident_causes(df_rescue, top_n=7):
    ...
    ax.set_title('주요 사고원인 비율', fontsize=16, pad=20) # 요청사항 4
    ax.axis('equal')
    ax.legend(top_causes_pie.index, title="사고원인", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1), fontsize=9)
    plt.tight_layout(rect=[0,0,0.85,1]); st.pyplot(fig) # 범례 공간 확보
...
def plot_correlation_scatter_ratio(merged_df, x_col_ratio, y_col_incident, title_text, x_label_text, selected_cause):
    ...
    # 요청사항 7: x_label_text에서 "(65세 이상)" 제거 (함수 호출부에서 조절)
    ...
    ax.set_xlabel(x_label_text, fontsize=12) # 원본 x_label_text 사용
    ax.set_ylabel(f'{selected_cause}건수 (건)', fontsize=12) # 요청사항 6
...
def plot_bubble_chart_ratio(df_final_merged, target_cause_for_bubble):
    ...
    ax.set_title(f'고령 인구 비율, 노후 주택 비율과 {target_cause_for_bubble} 발생 건수', fontsize=16, pad=15) # 제목도 업데이트
...
def plot_heatmap_housing_elderly_incident_ratio(df_merged_ratio, accident_col_name, target_safety_accident_cause):
    ...
    try:
        ...
        heatmap_data_ratio = df_merged_ratio_copy.pivot_table(
            ...
            index='노후주택비율_bin_label', # Y축이 될 컬럼
            columns='고령인구비율_bin_label', # X축이 될 컬럼
            aggfunc='mean',
            observed=True 
        ).sort_index(ascending=False) # Y축은 보통 위에서 아래로 값이 커지므로, 비율이므로 False 유지 또는 True로 변경 고려
        ...
        if not heatmap_data_ratio.empty:
            ...
            ax_heatmap.set_title(f'고령 인구 비율 및 노후 주택 비율 구간별 평균 {target_safety_accident_cause} 발생 건수', fontsize=16, pad=15) # 제목 업데이트
...
def run_housing_safety_page():
    ...
    # 요청사항 3: 탭 이름 변경
    tab_titles_ordered = ["구별 사고 건수", "구별 사고 원인", "주요 사고 원인", "시간대별 사고", "상관관계 분석", "통합 상관관계 분석"]
    tabs = st.tabs(tab_titles_ordered)

    with tabs[0]: 
        st.subheader("구별 총 사고 발생 건수")
        plot_gu_incident_counts(df_rescue_processed) # Y축 레이블은 함수 내에서 수정됨

    with tabs[1]: 
        st.subheader("구별 사고원인별 발생 건수")
        top_n_causes_stacked = st.slider("표시할 상위 사고원인 개수:", 3, 15, 7, key="stacked_bar_top_n_slider_housing_main_v2")
        plot_stacked_bar_incident_causes_by_gu(df_rescue_processed, top_n_causes=top_n_causes_stacked) # Y축 레이블은 함수 내에서 수정됨

    with tabs[2]: 
        st.subheader("서울시 전체 주요 사고원인 비율") # 탭 이름과 별개로 부제목은 유지 가능 (또는 변경 가능)
        top_n_causes_pie = st.slider("표시할 상위 사고원인 개수:", 3, 10, 7, key="pie_chart_top_n_slider_housing_main_v2")
        plot_pie_major_incident_causes(df_rescue_processed, top_n=top_n_causes_pie) # 그래프 제목은 함수 내에서 수정됨

    with tabs[3]: 
        st.subheader("시간대별 사고 발생 추이")
        if '신고시각' in df_rescue_processed.columns:
            df_rescue_time_analysis = df_rescue_processed.copy()
            # 시간 형식 변환 시도 (다양한 형식 고려)
            try:
                df_rescue_time_analysis['신고시간_dt'] = pd.to_datetime(df_rescue_time_analysis['신고시각'], errors='coerce')
            except Exception: # 좀 더 일반적인 예외 처리
                 df_rescue_time_analysis['신고시간_dt'] = pd.NaT # 변환 실패 시 NaT로
            ...
            if '신고시간_dt' in df_rescue_time_analysis.columns and not df_rescue_time_analysis['신고시간_dt'].isnull().all():
                ...
                hourly_incidents_counts_df = df_rescue_time_analysis.dropna(subset=['신고시간(시)']) # 컬럼명 변경에 따른 수정
                ...
                if not hourly_incidents_counts_df.empty:
                    ...
                    hourly_incidents_final_counts = hourly_incidents_counts_df['신고시간(시)'].value_counts().sort_index() # 변수명 변경
                    if not hourly_incidents_final_counts.empty:
                        ...
                        ax_time.set_ylabel('사고 건수 (개)', fontsize=12) # 요청사항 5
        ...
        if '사고원인' in df_rescue_processed.columns and \
           ...
           '노후주택비율' in df_housing_processed_h.columns:
            ...
            selected_cause_corr_ratio = st.selectbox(
                "상관관계 분석 사고 원인:", # 레이블에서 (비율) 제거
                unique_causes_list_h_corr_ratio, 
                index=default_idx_corr_ratio, 
                key="corr_ratio_cause_select_housing_v2"
            )
            ...
            if selected_cause_corr_ratio:
                ...
                if not merged_df_corr_elderly_ratio.empty:
                    plot_correlation_scatter_ratio(
                        ...
                        "고령 인구 비율", # 요청사항 7: "(65세 이상)" 제거됨
                        selected_cause_corr_ratio # 범례 및 y축 레이블에 사용될 원인명
                    )
                else: st.info("고령 인구 비율 데이터와 사고 건수 병합 결과 데이터가 없습니다.")
                # 요청사항 9: 그래프 사이의 선 삭제 (st.divider() 호출 안 함)
    ...
    with tabs[5]: 
        ...
        cause_for_bubble_ratio = st.selectbox(
            "버블/히트맵 기준 사고 원인:", # 레이블에서 (비율) 제거
            unique_causes_list_h_bubble_ratio, 
            index=default_idx_bubble_ratio, 
            key="bubble_ratio_cause_select_housing_v2"
        )
               ...
               not df_elderly_processed_h.empty and '고령인구비율' in df_elderly_processed_h.columns:
                ...
                if not df_merged_bubble_ratio_step1.empty:
                    ...
                    if not df_final_merged_for_bubble_ratio.empty: 
                        st.markdown("#### 버블 차트")
                        plot_bubble_chart_ratio(df_final_merged_for_bubble_ratio, cause_for_bubble_ratio) # X,Y축 함수 내 수정
                        st.divider()
                        st.markdown("#### 히트맵")
                        plot_heatmap_housing_elderly_incident_ratio(df_final_merged_for_bubble_ratio, f'{cause_for_bubble_ratio}건수', cause_for_bubble_ratio) # X,Y축 함수 내 수정
                    else: st.info(f"'{cause_for_bubble_ratio}' 사고 기준 종합 분석용 최종 데이터가 비어있습니다.") # (비율 기반) 제거
                ...
                st.info(f"다음 데이터가 준비되지 않아 버블 차트/히트맵 분석을 수행할 수 없습니다: {', '.join(missing_info_bubble)}") # (비율 기반) 제거
